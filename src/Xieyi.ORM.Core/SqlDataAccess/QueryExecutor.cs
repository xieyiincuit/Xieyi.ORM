...
namespace Xieyi.ORM.Core.SqlDataAccess
{
    internal class QueryExecutor
    {
        ...
        {
            if (!DbContext.RealExecutionSaveToDb) return default(int);
        ...
        {
            if (!DbContext.RealExecutionSaveToDb) return default(object);
        ...
        {
            if (!DbContext.RealExecutionSaveToDb) return default(DbDataReader);
            DbContext.ParameterInitializes();
            DbContext.CheckConnectionStatus();
        ...
        {
            if (!DbContext.RealExecutionSaveToDb) return default(DataTable);
        ...
        {
            if (!DbContext.RealExecutionSaveToDb) return default(DataSet);
        ...
        {
            if (!DbContext.RealExecutionSaveToDb) return default(List<Entity>);

            var dt = ds.Tables[0];
            if (dt.Rows.Count <= 0) return default(List<Entity>);
        ...
        {
            if (!DbContext.RealExecutionSaveToDb) return default(Entity);
            var dt = ds.Tables[0]; // 获取到ds的dt
            return dt.Rows.Count > 0 ? FillAdapter<Entity>.AutoFill(dt.Rows[0]) : default(Entity);
        }
    }
}