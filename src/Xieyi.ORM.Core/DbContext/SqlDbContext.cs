...
public abstract class SqlDbContext : DbContext, IExecuteSql, ICacheable
{
    ...
    internal override string GetQueryCacheKey()
    {
        ...
        if (Parameters != null && Parameters.Any()) return MD5Helper.GetMd5Hash($"{SqlStatement}_{string.Join("|", Parameters.Values)}");
    ...
    {
        return TableAttribute.GetName(typeof(TEntity));
    }
    ...
    {
        //获取下次执行的链接字符串
        var connectionString = ConnectionManager.SetAndGetConnectionString(operationType);
    ...
    public override void Add<TEntity>(TEntity entity)
    {
        ...
        CommandTextGenerator.Add(entity);
        SwitchConnection(OperationType.Write);
    ...
    {
        DbCommand.CommandType = CommandType.Text;
        CommandTextGenerator.Delete(filter);
        SwitchConnection(OperationType.Write);
    ...
    {
        DbCommand.CommandType = CommandType.Text;
        CommandTextGenerator.Delete(entity);
        SwitchConnection(OperationType.Write);

        QueryExecutor.ExecuteNonQuery();
        DbCacheManagerSafeExecute(m => m.Delete(entity));
    }
    ...
    {
        DbCommand.CommandType = CommandType.Text;
        CommandTextGenerator.Update(entity, filter);
        SwitchConnection(OperationType.Write);
    ...
    public void Update<TEntity>(TEntity entity) where TEntity : class
    {
        ...
        DbCommand.CommandType = CommandType.Text;
        CommandTextGenerator.Update(entity, out var filter);
        SwitchConnection(OperationType.Write);
    ...
    public ILinqQueryable<TEntity> Queryable<TEntity>() where TEntity : class
    {
        DbCommand.CommandType = CommandType.Text;
        //重置命令生成器，防止上次查询参数被重用
        CommandTextGenerator = CreateCommandTextGenerator();
        SwitchConnection(OperationType.Read);
    ...
    public IQueryable<TEntity> Queryable<TEntity>(string sqlStatement, IDictionary<string, object> @params = null) where TEntity : class
    {
        ...
        Parameters = @params;
        SwitchConnection(OperationType.Read);
        IsSqlStatementOrStoredProcedure = true;
        return new SqlQueryable<TEntity>(this);
    }
