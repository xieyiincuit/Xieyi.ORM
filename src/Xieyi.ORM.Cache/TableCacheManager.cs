...
namespace Xieyi.ORM.Cache
{
    internal class TableCacheManager : CacheManagerBase
    {
        ...
        {
            string key = $"{CacheOptionsConst.CacheKey_TableCache}_{DbContext.DataBaseName}_{collectionName ?? DbContext.CollectionName}";
        ...
        internal List<TEntity> GetEntitiesFromCache<TEntity>(Expression<Func<TEntity, bool>> filter) where TEntity : class
        {
            ...
            if (TableCachingAttribute.IsExistTaleCaching(typeof(TEntity), out TimeSpan tableCacheTimeSpan))
            {
                StartScanTable<TEntity>(tableCacheTimeSpan);
            }
        ...
        {
            string scanKey = $"{CacheOptionsConst.CacheKey_TableScanning}{DbContext.CollectionName}";
