...
internal class QueryCacheManager : CacheManagerBase
{
    ...
    /// </summary>
    /// <returns></returns>
    private string GetSqlQueryCacheKey()
    {
        return DbContext.GetQueryCacheKey();
    }
    ...
    {
        var key = $"{CacheOptionsConst.CacheKey_QueryCache}{collectionName ?? DbContext.CollectionName}";
    ...
    {
        var queryCacheKey = GetQueryCacheKey();
        var sqlQueryCacheKey = GetSqlQueryCacheKey();
